 ...
 *  adds 'wassup_compat_register_widget' function to emulate 'register_widget' for Wordpress 2.2 - 2.7
 *
...
$php_vers=phpversion();
//nothing to do here
if(version_compare($GLOBALS['wp_version'],'2.8','>=') && version_compare($php_vers,'5.2','>=')){
	return;
}
...
class Wassup_Widget{
	...
	var $number = false;
	/** constructor */
	function __construct($id_base="wassup_widget",$name="Wassup Widget",$widget_opts=array(),$control_opts=array()){
	...
	function wassup_widget($id_base="wassup_widget",$name="Wassup Widget",$widget_opts=array(),$control_opts=array()){
		$this->__construct($id_base,$name,$widget_opts,$control_opts);
	}
...
class wassup_topstatsWidget extends Wassup_Widget{
	function __construct(){
		...
		//instantiate parent
		parent::__construct($widget_id,$widget_name,$widget_opts,$control_opts);	//parent::__construct()
	}
	/** PHP4-compatible constructor */
