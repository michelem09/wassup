 ...
 *  - rewritten for PHP4 - 5.1 compatibility
 ...
 *   wassup_Dashboard_Widgets: dashboard chart and visitor summary
 *
 * @package	WassUp Real-time Analytics
 * @subpackage	compat_wassup.class.php module
...
//-------------------------------------------------
//## PHP4-compatible Wassup classes
...
class wassupOptions {
	...
	function get_apikey($api="googlemaps"){
		$apikey="";
	 ...
	 * @since v1.9.1
	 */
	function run_shell_cmd($cmd=false){
		$ret_value=false;
		if(!empty($cmd)){
...
class wassupDb{
			...
			foreach($dbtasks as $db_sql){
				...
				if(strpos($db_sql,"DELETE FROM `$table_prefix")!==false){
					...
					if(strpos($db_sql,$wassup_table)>0){
						$timestamp=time();
						$last_optimized=wassupDb::get_wassupmeta($wassup_table,'_optimize');
						if(empty($last_optimized) || ($timestamp - $last_optimized)>24*3600){
							//save timestamp to prevent repeat of optimize
							$expire=time()+7*24*3600;
							$wassup_optimized=wassupDb::update_wassupmeta($wassup_table,'_optimize',$timestamp,$expire);
							//do optimize
							$results=$wpdb->query($db_sql);
						}else{
	...
	function get_records($table,$startid=0,$condition="",$limit=0){
		global $wpdb,$wdebug_mode;
	...
	function export_records($table,$start_id,$wherecondition,$dtype="sql"){
...
class wassupURI {
	...
	function add_siteurl($inputurl){
		if(preg_match('/^https?\:/',$inputurl)===false){
			...
			return wassupURI::cleanURL($outputurl);
		}else{
			return wassupURI::cleanURL($inputurl); //security fix
		}
	}
	...
	function get_urldomain($urlparam=""){
		$domain=false;
		$url=array();
		//default param is this site's url
		if(empty($urlparam)) $urlparam=wassupURI::get_sitehome();
	...
	function url_link($urlrequested,$chars=0,$spam=0){
		...
		if(strlen($request)>60) $tooltip=' title="'.wassupURI::cleanURL($request).'" ';
		else $tooltip="";
		if($chars >0) $cleaned_uri=stringShortener("$urlrequested",round($chars*.9,0));
		else $cleaned_uri=wassupURI::cleanURL("$urlrequested");
	...
	function referrer_link($wrec,$chars=0){
		global $wassup_options;
		...
		if(!empty($referer)){
			...
			if(strlen($ref)>60) $tooltip=' title="'.wassupURI::cleanURL($ref).'" ';
			if($chars >0) $cleaned_uri=stringShortener("$referer",round($chars*.9,0));	//v1.9.4 bugfix
			else $cleaned_uri=wassupURI::cleanURL("$referer");
			...
			if(stristr($referer,$wpurl)==$referer || stristr($referer,$siteurl)==$referer){
				...
				if(!empty($wrec->urlrequested) && ($ref == $siteurl.$wrec->urlrequested || $ref == rtrim($siteurl.$wrec->urlrequested,'/'))){
					...
					if(strpos($ref,'wp-login.php')>0 || strpos($ref,'/wp-includes/')>0 || strpos($ref,'/wp-content/')>0){
						$referrerlink='<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'>'.$cleaned_uri.'</a>';
					}else{
			...
			//external referrer
				...
				if($spam==2 || wassupURI::is_xss($ref)){
					$referrerlink='<span class="malware"'.$tooltip.'>'.$cleaned_uri.'</span>';
	...
	function se_link($wrec,$chars=0,$keywords=""){
		global $wassup_options;
		...
		if(!empty($referer)){
			$tooltip="";
			$ref=strtolower($referer);
			if(strlen($ref)>60) $tooltip=' title="'.wassupURI::cleanURL($referer).'" ';
			if(empty($spam) && preg_match('/\/wp\-(?:admin|content|includes)\/|\/wp\-login\.php/i',$ref)==0 && !wassupURI::is_xss($ref)){
				if(!empty($keywords)){
					$selink='<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'><span>'.esc_attr($keywords).'</span></a>';
				}else{
					if($chars >0) $selink='<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'><span>'.stringShortener($referer,round($chars*.8,0)).'</span></a>';
					else $selink='<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'><span>'.wassupURI::cleanURL($referer).'</span></a>';
				}
			}
		}
		return $selink;
	 ...
	 * @since v1.9
	 */
	function disarm_attack($urlstring=false){
		if(!empty($urlstring) && !is_numeric($urlstring)){
	...
	function cleanURL($url=""){
		if(!empty($url) && !is_numeric($url)){
			...
			if(preg_match('#^(/|\?|http)#',$url)>0){
				$cleaned=str_replace(array('&#038;','&#38;','&amp;'),'&',esc_url(wassupURI::disarm_attack($url)));
			}else{
				$allowed=array('http','https','ftp','ftps','mailto','news','irc','gopher','nntp','feed','telnet','mms','rtsp','svn','tel','fax','xmpp','webcal');
				$cleaned=wp_kses_bad_protocol(wassupURI::disarm_attack($url),$allowed);
			}
		}else{
			$cleaned=$url;
		}
	...
	function get_admin_url($link_part){
		$url="";
		if(!empty($link_part)){
	...
	function get_ajax_url($action=""){
		global $wp_version;
		$ajaxurl=wassupURI::get_admin_url('admin-ajax.php');
		if(version_compare($wp_version,'2.7','<')){
			...
			$ajaxurl=wassupURI::get_admin_url('index.php?page=wassup-stats');
		}elseif($action=="Export"){
			$ajaxurl=wassupURI::get_admin_url('index.php?page=wassup-options&tab=3');
		}
		return $ajaxurl;
	}
} //end Class wassupURI
...
class wassupIP {
	 ...
	 * @return string
	 */
	function get_clientAddr($ipAddress=""){
		$proxy="";
		$hostname="";
		$IP="";
		...
		if(!empty($IP)){
			...
			if(!empty($proxylist)) $proxy=wassupIP::validIP($proxylist);
	...
	function get_hostname($IP=""){
		if(empty($IP)) $IP=wassupIP::clientIP($_SERVER['REMOTE_ADDR']);
...
/**
 * PHP4-compatible static class container for WassUp dashboard widgets functions
 * @since v1.9
