<?php
/**
 * PHP4 - 5.1 compatibility functions and classes for Wassup.
 *  - defines missing PHP5 function used by Wassup
 *  - recreates Wassup's PHP5 classes as PHP4-compatible classes
 *
 * @package	WassUp Real-time Analytics
 * @subpackage	compat_php.php module
...
unset($wfile);
//-------------------------------------------------
//## Missing PHP5 functions used by Wassup 
if(!function_exists('json_decode')){	//function added in PHP 5.2
	/** crude emulation of the PHP 5.2+ function to load remote data into Wassup */
			...
			foreach($dbtasks as $db_sql){
				...
				if(strpos($db_sql,"DELETE FROM `$table_prefix")!==false){
					...
					if(strpos($db_sql,$wassup_table)>0){
						$timestamp=time();
						$last_optimized=wassupDb::get_wassupmeta($wassup_table,'_optimize'); //v1.9.1 bugfix
...
class wassupURI {
	...
	function add_siteurl($inputurl){
		if(preg_match('/^https?\:/',$inputurl)===false){
			...
			return wassupURI::cleanURL($inputurl);	//v1.9.1 security fix
		}
	}
	...
	function url_link($urlrequested,$chars=0,$spam=0){
		...
		if(!empty($spam) || wassupURI::is_xss($urlrequested)){
			...
			else $urllink .=wassupURI::cleanURL("$urlrequested");
			$urllink .='</span>';
			...
			$urllink='<a href="'.wassupURI::add_siteurl($request).'" target="_BLANK">';
		...
		if(!empty($referer)){
			...
			if(stristr($referer,$wpurl)==$referer || stristr($referer,$siteurl)==$referer){
				...
				if(!empty($wrec->urlrequested) && $ref == $siteurl.$wrec->urlrequested){
					...
					if(strpos($ref,'wp-login.php')>0 || strpos($ref,'/wp-includes/')>0 || strpos($ref,'/wp-content/')>0){
						$referrerlink='<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'>'.stringShortener($referer,round($chars*.9,0)).'</a>';
					}else{
			...
			//external referrer
				...
				if($spam==2 || wassupURI::is_xss($ref)){
					...
					$referrerlink=$favicon_img.'<a href="'.wassupURI::cleanURL($referer).'" target=_"BLANK"'.$tooltip.'>'.stringShortener($referer,round($chars*.9,0)).'</a>';
				}
			}
		}else{
			$referrerlink='<span>'.__("direct hit","wassup").'</span>';
